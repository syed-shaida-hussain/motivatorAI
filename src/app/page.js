...
export default function Home() {
	...
	return (
		<div
			className={
				isDark ? 'dark flex flex-col h-screen' : 'light flex flex-col h-screen'
			}
		>
			<main className=" bg-backgroundColor text-textColor flex flex-col items-center gap-5 pt-20">
				...
				<motion.h1
					...
					className="text-3xl md:text-5xl lg:text-7xl tracking-wide px-3 text-center"
				>
					Introducing MotivatorAI
				</motion.h1>
				<div className="flex flex-col gap-7 sm:flex-row  items-center my-10 font-medium">
					<Link
						href="/chat-page"
						className="flex gap-1 items-center bg-textColor text-backgroundColor px-6 py-2 rounded-full"
					>
						Try MotivatorAI <MdArrowOutward className="h-4 w-4" />
					</Link>
				</div>
